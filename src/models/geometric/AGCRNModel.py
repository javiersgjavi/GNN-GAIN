...
class GNN(nn.Module):
    ...
    def __init__(self, periods, nodes, edge_index, edge_weights, batch_size):
        '''AVISO: Este modelo no usa ni pesos ni vertices'''
        super().__init__()

        self.model = AGCRNModel(
            input_size=2,
            hidden_size=nodes,
            output_size=1,
            horizon=periods,
    ...
    def forward_g(self, x: torch.Tensor, input_mask: torch.Tensor) -> Tuple[Tensor, Tensor]:
        ...
        imputation = self.model(input_tensor).squeeze(dim=-1)
    ...
    def forward_d(self, x: torch.Tensor, hint_matrix: torch.Tensor) -> torch.Tensor:
        ...
        pred = self.model(input_tensor).squeeze(dim=-1)
