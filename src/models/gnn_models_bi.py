...
from tsl.nn.blocks import ConditionalBlock
...
class BaseGNN(nn.Module):
    ...
    def bi_forward(self, input_tensor_f, input_tensor_b, edges, weights, exog=None):
        #exog_b = torch.flip(exog, dims=[1]) if exog is not None else None
        ...
        if exog is None:
            ...
            output= torch.sigmoid(self.decoder_mlp(h, u=exog))
        return output
    ...
    def forward_g(self, x: torch.Tensor, input_mask: torch.Tensor, time_gap_matrix: torch.Tensor, exog=None) -> Tuple[
        ...
        exog = exog[:,:,:,:2] if exog is not None else None
        imputation = self.bi_forward(input_tensor_f, input_tensor_b, self.edge_index, self.edge_weights, exog).squeeze(dim=-1)
    ...
    def forward_d(self, x: torch.Tensor, hint_matrix: torch.Tensor, exog=None) -> torch.Tensor:
        ...
        exog = exog[:,:,:,:2] if exog is not None else None
        pred = self.bi_forward(input_tensor_f, input_tensor_b, self.edge_index, self.edge_weights, exog=exog).squeeze(dim=-1)
        return pred
...
class GRUGCNBI(BaseGNN):
    ...
    def __init__(self, args, time_gap_matrix=False, exog=False):
        ...
        self.model_f = GRUGCNModel(
            exog_size= 0 if not self.exog else 4,
...
class RNNEncGCNDecBI(BaseGNN):
    ...
    def __init__(self, args, time_gap_matrix=False, exog=False, gen=False):
        super().__init__(edge_index=args['edge_index'], edge_weights=args['edge_weights'])
        ...
        if not self.exog:
            ...
            self.decoder_mlp = ConditionalBlock(
                input_size=self.output_size*2,
                exog_size=2,
                output_size=1,
                activation='linear',
