...
class RNN(nn.Module):
    ...
    def __init__(self, periods, nodes):
        ...
        self.rnn1 = nn.LSTM(input_size=periods, hidden_size=nodes, batch_first=True, dropout=0.0, num_layers=1, bidirectional=True).apply(init_weights_xavier)
        self.rnn2 = nn.LSTM(input_size=periods, hidden_size=nodes, batch_first=True, dropout=0.0, num_layers=1).apply(init_weights_xavier)
        self.fc = nn.Linear(nodes, nodes).apply(init_weights_xavier)
